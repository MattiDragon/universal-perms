...
// UNUSED: import java.util.function.Predicate;
...
public class UniversalPerms implements ModInitializer {
    ...
    public void onInitialize() {
        CommandRegistrationCallback.EVENT.addPhaseOrdering(Event.DEFAULT_PHASE,
                new Identifier("universal_perms", "after"));
        CommandRegistrationCallback.EVENT.register(new Identifier("universal_perms", "after"),
    ...
    private static void alterNode(CommandNode<ServerCommandSource> node, Deque<String> location,
            Map<CommandNode<ServerCommandSource>, String> visited) {
        ...
        if (visited.containsKey(node)) {
            LOGGER.warn("Encountered same node a two different places, '{}' and '{}'. This shouldn't happen!",
                    visited.get(node), nodeLocation);
            if (!name.isEmpty())
                location.removeLast();
            return;
        }
        ...
        ((CommandNodeAccess) node)
                .setRequirement((ServerCommandSource source) -> (source.getEntity() == null ? TriState.DEFAULT
                        : Permissions.getPermissionValue(source, permission))
                        .orElseGet(() -> requirement.test(source)));
